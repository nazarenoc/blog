...
mongoose.connect('mongodb://user:user123456@ds019470.mlab.com:19470/blogdb', function (err) {
    if (err) {
    ...
    if (req.body.isNew) {
            ...
            .exec(function(err, user){
                ...
                if (user != null) {
                    ...
                    newUser.save(function(err){
                        if (err) res.send(err);
                        var token = jtw.sign(newUser, secretKey, {expiresIn: "1 day"});
        ...
        User.findOne({ login: req.body.login, password: req.body.password }, 'name')
            .exec(function(err, user){
                if (err) req.send(err);
                if (user != null) {
                    var token = jwt.sign(user, secretKey, {expiresIn: "1 day"});
